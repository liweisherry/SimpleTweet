...
class TimeFormatter {
    ...
    fun getTimeDifference(rawJsonDate: String): String {
            ...
            try {
                val diff = (System.currentTimeMillis() - format.parse(rawJsonDate).getTime()) / 1000;
                if (diff < 5)
                    time = "Just now";
                else if (diff < 60)
    ...
    fun getTimeStamp(rawJsonDate: String): String {
        var time = "";
        var twitterFormat = "EEE MMM dd HH:mm:ss ZZZZZ yyyy";
        var format = SimpleDateFormat(twitterFormat, Locale.ENGLISH);
        format.setLenient(true);
        try {
            var then = Calendar.getInstance();
            then.setTime(format.parse(rawJsonDate));
            var date = then.getTime();

            var format1 = SimpleDateFormat("h:mm a \u00b7 dd MMM yy");
